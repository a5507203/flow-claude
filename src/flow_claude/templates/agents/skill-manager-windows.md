...
## Initialization Context (Windows)

Due to Windows CLI limitations, your system prompt does NOT include the skill catalog. You must:

1. **Read this workflow document first** - You are reading it now
2. **Read skill files manually** - Load SKILL.md files from `src/flow_claude/templates/skills/*/SKILL.md`

### Loading Skills

Before processing tasks, read all available skill files:

```bash
# List available skills
ls src/flow_claude/templates/skills/
```

Then read each SKILL.md file to understand available sections:

```bash
cat src/flow_claude/templates/skills/git-tools/SKILL.md
cat src/flow_claude/templates/skills/launch-workers/SKILL.md
cat src/flow_claude/templates/skills/your-workflow/SKILL.md
```

Read these files into your context before mapping tasks to skill sections.
...
After reading the SKILL.md files, each skill follows this structure:
...
- **Read skill files first** - You must read the SKILL.md files before mapping tasks
...
First, load all available skill files into your context:
...
Note the section headers (e.g., "Shared Definitions", "Read Tools", "Write Tools") available in each skill.

### Step 2: Read Task Summaries
...
### Step 5: Map Task to Skill Sections

Reference the skill files you read in Step 1 and determine needed sections:
...
### Step 6: Extract Skills for Worker
...
Note available sections: "Shared Definitions", "Read Tools", "Write Tools", etc.

### 2. Check Task Summaries
...
Reference skill files you read in Step 1 for available sections.
...
- Re-read the relevant SKILL.md file to verify correct header names
...
- **Read skill files first** - On Windows, skill catalog is NOT pre-loaded; read SKILL.md files in Step 1
